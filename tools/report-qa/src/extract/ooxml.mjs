...
/**
 * Read a ZIP archive into a Map of entry name -> the entry exactly as stored:
 * its method, CRC, sizes and still-compressed bytes.
 *
 * This is what lets a package be rebuilt without a DEFLATE encoder. Every part
 * we are not changing is copied straight back out, so it keeps its compression
 * and, just as importantly, its bytes - nothing is re-encoded behind the
 * author's back.
 */
export function readZipEntries(input) {
  ...
  for (let i = 0; i < entryCount; i += 1) {
    ...
    const crc = readU32(buffer, pointer + 16);
    const compressedSize = readU32(buffer, pointer + 20);
    const uncompressedSize = readU32(buffer, pointer + 24);
    ...
    entries.set(name, {
      ...
      compressedSize,
      uncompressedSize,
      bytes: buffer.subarray(dataStart, dataStart + compressedSize),
    });
